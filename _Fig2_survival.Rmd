...
for(location in c("CA", "AU", "US")){
    ...
    for(target in c("delta", "rho", "ETM")){
        ## load weights
        sc.file <- paste0("./", target, "_weight.csv.gz")
...
p_meta_combined <- c()
for(target in c("delta", "rho", "ETM")){
        coxph_res_AU <- fread(paste0("survival/coxph_res_AU_", target,".txt"))
        coxph_res_CA <- fread(paste0("survival/coxph_res_CA_", target,".txt"))
        coxph_res_US <- fread(paste0("survival/coxph_res_US_", target,".txt"))
...
p <- ggplot(data= df_meta, aes(x=coef, y=-log10(p), col=diffexpressed, label=topic)) + 
        ...
        geom_vline(xintercept=c(-0.0015, 0.0015), col="red") +
        geom_hline(yintercept=-log10(0.05), col="red")+
        scale_x_continuous(labels = function(x) format(x, scientific = TRUE)) +
        theme(panel.spacing = unit(2, "lines")) +
        xlab("Harzard ratio") + ylab("Significance(-log10)") + ggtitle("PDAC survival analysis
across three different cohorts
")
p
ggsave(paste0("survival/figures/meta_volcano.pdf"), p)
```